...
const UserMenu = ({
  ...
  <DropdownMenu>
    <DropdownMenuTrigger asChild>
      <Button variant="ghost" className="p-0 rounded-4xl">
        <Avatar>
          <AvatarImage src={userAvatar} alt={userName} className="h-7 w-7" />
    ...
    <DropdownMenuContent align="end" className="w-56">
      ...
      <DropdownMenuSeparator />
      <DropdownMenuItem>
        <Link to="/profile" className="no-underline w-full block">
          Min Profil
        </Link>
      </DropdownMenuItem>
      {isAdmin && (
        <DropdownMenuItem>
          <Link to="/godaddy" className="no-underline w-full block">
            Admin Dashboard
          </Link>
        </DropdownMenuItem>
      )}
      <DropdownMenuSeparator />
      <DropdownMenuItem onClick={() => handleLogout && handleLogout()}>Log out</DropdownMenuItem>
    </DropdownMenuContent>
  </DropdownMenu>
);
                      ...
                      {navigationLinks.map((link, index) => (
                        <NavigationMenuItem key={index} className="w-full">
                          <button
                            ...
                            className="flex w-full items-center rounded-md px-3 py-2  font-medium transition-colors hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground cursor-pointer no-underline"
                          >
                            {link.label}
                          </button>
                        </NavigationMenuItem>
                  ...
                  {navigationLinks.map((item) => {
                    return (
                      <Link
                        ...
                        to={item.href}
                        activeOptions={{
                          exact: item.href === "/godaddy",
                        }}
                        activeProps={{ className: "text-black" }}
                        inactiveProps={{ className: "text-gray-400 hover:text-gray-700" }}
                        className={`inline-flex items-center p-1`}
                        // className={`inline-flex items-center px-1 pt-1  font-medium ${
                        //   isActive ? "text-black" : "text-gray-400 hover:text-gray-700 hover:border-gray-300"
                        // }`}
                      >
                        {item.icon}
